...
from main import (
    ...
    set_survival_rate, get_current_config, create_schedule_task, 
    load_schedule_from_json, schedule_to_dict, update_genetic_config
)
...
async def visualize_schedule_route():
    try:
        ...
        return APIResponseModel(
            ...
            schedule=raw_data  # Просто передаем весь raw_data
        )
...
async def update_genetic_config_route(config: GeneticConfigModel):
    try:
        update_genetic_config(config)
...
async def get_genetic_config_route():
    try:
